...
function addBookmark() {
    checkUserLoggedIn().then(() => {
        function setBookmarkBackground(bookmarkElement, articleId) {
            ...
            if (clickCount === 1) {
                bookmarkElement.style.background = `url("../Front-Enter/image/star-background.svg") center / cover no-repeat`;
            } else {
                bookmarkElement.style.background = `url("../Front-Enter/image/star-border.svg") center / cover no-repeat`;
            }
        }
